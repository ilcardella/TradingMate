...
def test_compute_avg_holding_open_price(portfolio):
    price = portfolio.compute_avg_holding_open_price(
        "MOCK13", portfolio.get_trade_history()
    )
    assert price == 166.984
    price = portfolio.compute_avg_holding_open_price(
        "MOCK4", portfolio.get_trade_history()
    )
    assert price == 582.9117
    price = portfolio.compute_avg_holding_open_price(
        "MOCK", portfolio.get_trade_history()
    )
